// Package fields_bn254 implements the BN254 degree-12 extension field 𝔽p¹²
// in-circuit, on top of the polynomial ring checker in the root package.
//
// 𝔽p¹² is represented directly as 𝔽p[x]/(x¹² - 18x⁶ + 82) rather than as the
// usual quadratic-over-cubic-over-quadratic tower. The direct representation is
// what lets a full 𝔽p¹² multiplication become a single polynomial ring product
// check: the prover supplies the remainder and the quotient of the Euclidean
// division through a hint, and all the checks of a circuit are batched into one
// deferred identity test at a random point.
...
type basePoly = polyring.Poly[emulated.BN254Fp]
type curveF = emulated.Field[emulated.BN254Fp]
type baseEl = emulated.Element[emulated.BN254Fp]

// E2 is an element of 𝔽p², used only as an intermediate representation when
// converting to and from the gnark-crypto tower (see [Ext12.ToTower]).
type E2 struct {
	A0, A1 baseEl
}

// Ext2 holds the 𝔽p² arithmetic, including the constants needed for the
...
// NewExt2 returns an 𝔽p² arithmetic context with its own emulated field.
func NewExt2(api frontend.API) *Ext2 {
	fp, err := emulated.NewField[emulated.BN254Fp](api)
	if err != nil {
		panic(err)
	}
	return newExt2(api, fp)
}

// newExt2 returns an 𝔽p² arithmetic context sharing the given emulated field.
func newExt2(api frontend.API, fp *curveF) *Ext2 {
...
func (e Ext2) Mul(x, y *E2) *E2 {
	// b0 = x0*y0 - x1*y1
	b0 := e.fp.Eval([][]*baseEl{{&x.A0, &y.A0}, {&x.A1, &y.A1}}, []int{1, -1})
	// b1 = x0*y1 + x1*y0
...
func (e Ext2) Square(x *E2) *E2 {
	...
	a := e.fp.Eval([][]*baseEl{{&x.A0, &x.A0}, {&x.A1, &x.A1}}, []int{1, -1})
	// b = 2*x0*x1
...
// FromE2 converts a gnark-crypto 𝔽p² element into its in-circuit witness form.
...
func (e Ext2) Inverse(x *E2) *E2 {
	...
	// 1 == inv * x
	e.AssertIsEqual(e.One(), e.Mul(&inv, x))
