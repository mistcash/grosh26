...
modulus and a coefficient field are all it sees — so it is not in this
repository at all. It lives in
[`mistcash/polynomial-ring-toolkit`](https://github.com/mistcash/polynomial-ring-toolkit),
with its own tests, docs and release notes, and grosh26 depends on it like
any other module. This repository is its reference consumer:
`std/ring_bn254` instantiates it once, for `x¹² − 18x⁶ + 82`.

Because that module pins gnark to the same fork this repository does, and Go
ignores `replace` in dependencies, the `replace` directive below is what
makes both agree on one gnark.
...
[the toolkit's own review spec](https://github.com/mistcash/polynomial-ring-toolkit/blob/main/docs/review-spec.md).
